...
public class pksAdapterPPU extends RecyclerView.Adapter<pksAdapterPPU.HolderData>{
    private Context ctx;
    private List<DataPPU> listppu;

    public pksAdapterPPU(Context ctx, List<DataPPU> listppu) {
        this.ctx = ctx;
        this.listppu = listppu;
    }

    @NonNull
    @Override
    public HolderData onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View layout = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_ppu, parent, false);
    ...
    public void onBindViewHolder(@NonNull HolderData holder, int position) {
        DataPPU dm = listppu.get(position);
